...
// ###########
//
// Depth-only shadow pass for the instanced bird flock. Mirrors the worldPos
// transform in bird.wgsl vertexMain, then projects into light space so birds
// cast moving shadows across the field.
...
fn vertexMain(input: VertexInput) -> @builtin(position) vec4f {
  ...
  return frame.lightSpaceMatrix * vec4f(worldPos, 1.0);
}
